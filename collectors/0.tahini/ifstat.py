...
def main():
    ...
    while True:
        ...
        for line in f_netdev:
            ...
            for i in range(8):
                if FIELDS[i]:
#                    print ("proc.net.%s %d %s iface=%s direction=in host=%s"
#                           % (FIELDS[i], ts, stats[i], m.group(1), os.uname()[1]))
#                    print ("proc.net.%s %d %s iface=%s direction=out host=%s"
#                           % (FIELDS[i], ts, stats[i+8], m.group(1), os.uname()[1]))
