    ...
    while True:
        ...
        for line in f_diskstats:
            ...
            if len(values) == 14:
                # full stats line
                for i in range(11):
		    if device in ("sdj","sdk","sdb","sdc"):
                    	print ("%s%s %d %s dev=%s fs=ue" % (metric, FIELDS_DISK[i], ts, values[i+3], device))
		    if device in ("sdl","sdm","sdd","sde"):
                    	print ("%s%s %d %s dev=%s fs=uo" % (metric, FIELDS_DISK[i], ts, values[i+3], device))
